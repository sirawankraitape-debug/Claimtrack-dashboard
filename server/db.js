...
const DEFAULT_STATUSES = [
  ...
  { key:'sokbo',         label:'เคส ศคบ.',                            color:'#0EA5E9', grp:'ดำเนินการ',    terminal:false, sort_order:11 },
  { key:'accident',      label:'รถเกิดอุบัติเหตุ',                     color:'#CA8A04', grp:'ดำเนินการ',    terminal:false, sort_order:12 },
  { key:'kex_claim',     label:'เคลม KEX',                            color:'#4F46E5', grp:'ดำเนินการ',    terminal:false, sort_order:13 },
  { key:'fraud',         label:'เคสทุจริต',                          color:'#DC2626', grp:'ดำเนินการ',    terminal:true,  sort_order:14 },
  { key:'closed',        label:'ปิดจบไม่เคลม',                      color:'#10B981', grp:'ดำเนินการ',    terminal:true,  sort_order:15 },
];
...
const init = async () => {
  ...
  // ลบสถานะ A00-A89, B00-B05 ที่เลิกใช้แล้ว (ทำครั้งเดียวก็พอ แต่ปลอดภัยถ้ารันซ้ำ)
  await pool.query(
    `UPDATE cases SET bill_status = NULL WHERE bill_status = ANY($1::text[])`,
    [REMOVED_STATUS_KEYS]
  );
  await pool.query(
    `DELETE FROM statuses WHERE key = ANY($1::text[])`,
    [REMOVED_STATUS_KEYS]
  );
